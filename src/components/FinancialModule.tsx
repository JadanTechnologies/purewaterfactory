...
export default function FinancialModule({
  ...
  const printTarget = typeof document !== 'undefined' ? document.getElementById('pos-print-section') : null;
  ...
  const handlePreviewReceipt = () => {
    const receiptHtml = document.getElementById('financial-audit-receipt')?.innerHTML;
    if (!receiptHtml) {
      alert('No receipt available to preview.');
      return;
    }
    ...
    previewWindow.document.write(`
      <html>
        <head>
          <title>Audit Receipt Preview</title>
          <style>
            body { margin: 20px; font-family: ui-monospace, monospace; color: #111; background: #fff; }
            .receipt-container { max-width: 680px; margin: 0 auto; }
            .header { text-align: center; margin-bottom: 16px; }
            .header h1 { font-size: 18px; margin: 0 0 4px; text-transform: uppercase; }
            .header p { font-size: 12px; margin: 2px 0; }
            .section { margin-bottom: 14px; }
            .section strong { display: inline-block; min-width: 140px; }
            .description { padding: 10px; border: 1px dashed #222; margin-top: 8px; white-space: pre-wrap; }
        ...
        <body>
          <div class="receipt-container">
            <div class="header">
              ...
              <p>Secure Audit Record System</p>
              <p>Transaction Audit Receipt</p>
            </div>
            <div class="section">${receiptHtml}</div>
            <div class="footer">Preview generated by PWFMS — use Print from the browser controls to output this receipt.</div>
          </div>
        </body>
      </html>
    `);
      ...
      {selectedTxn && (
        <>
          {printTarget && createPortal(
            <div id="financial-audit-receipt-printout" className="p-4 text-[10px] font-mono text-slate-900">
              <div style={{ textAlign: 'center', marginBottom: '12px' }}>
                <div style={{ fontWeight: 800, textTransform: 'uppercase', marginBottom: '4px', letterSpacing: '1px' }}>NILE PREMIUM WATER</div>
                <div style={{ fontSize: '9px', marginBottom: '2px' }}>SECURE AUDIT RECORD SYSTEM</div>
                <div style={{ fontSize: '9px', marginTop: '6px', borderTop: '1px dashed #000', borderBottom: '1px dashed #000', padding: '6px 0' }}>Transaction Audit Receipt</div>
              </div>
              <div style={{ lineHeight: 1.4 }}>
                <div><strong>Transaction ID:</strong> {selectedTxn.id}</div>
                <div><strong>Date/Time Logged:</strong> {selectedTxn.date}</div>
                <div><strong>Reference Token:</strong> {selectedTxn.reference}</div>
                <div><strong>Audit Type:</strong> {selectedTxn.type.toUpperCase()}</div>
                <div><strong>Counter-Party / Category:</strong> {selectedTxn.party}</div>
                <div style={{ margin: '8px 0', padding: '8px', border: '1px dashed #000' }}>
                  <strong>Transaction Description:</strong>
                  <div>{selectedTxn.description}</div>
                </div>
                <div><strong>Payment Gateway/Method:</strong> {selectedTxn.method}</div>
                <div><strong>Authorizing Officer:</strong> {selectedTxn.operator}</div>
                <div style={{ marginTop: '10px', borderTop: '1px dashed #000', paddingTop: '8px', display: 'flex', justifyContent: 'space-between' }}>
                  <span><strong>LEDGER VALUE:</strong></span>
                  <span>{currency}{selectedTxn.amount.toLocaleString()}</span>
                </div>
                {selectedTxn.type === 'Sale' && (
                  <div style={{ marginTop: '8px', padding: '8px', border: '1px dashed #000' }}>
                    <div>Invoice Status: <strong>{selectedTxn.status}</strong></div>
                    <div>Direct Paid: <strong>₦{selectedTxn.amountPaid.toLocaleString()}</strong></div>
                  </div>
                )}
              </div>
            </div>,
          ...
          <div className="fixed inset-0 z-50 bg-black/75 backdrop-blur-xs flex items-center justify-center p-4">
            <div className="bg-slate-900 border border-slate-700 w-full max-w-md rounded-2xl p-6 shadow-2xl animate-in scale-in duration-150 relative">
              ...
              {/* Receipt Body */}
              <div id="financial-audit-receipt" className="mt-4 space-y-3.5 text-xs">
                <div className="flex justify-between">
              ...
              <div className="mt-6 pt-4 border-t border-slate-800 flex gap-2">
                <button
                  ...
                  className="flex-1 py-2 bg-slate-700 hover:bg-slate-600 text-white font-semibold text-xs rounded-xl cursor-pointer flex items-center justify-center gap-1.5 transition-all uppercase tracking-wider shadow-md"
                >
